...
<%}%>
<%
ResultSet s3=db.exquery("select distinct gname,group_id,user_id,status from tbl_group where user_id='"+id+"' and status='1'");
String group="";
while(s3.next())
{
group=s3.getString("gname");
%>
<li>&#8227; <a href="group_chat.jsp?g_id=<%=s3.getString("group_id")%>" class="a_top_hypers"><%=group%></a></li>
<%}%>
</ul>
 ...
 <%
String gname=request.getParameter("id"); 
%> 
<div class="service-features wow fadeInRight">
    <center>
    <div style="border: solid; width: 500px;">
   <table   cellpadding="8" class="formtable">                           
                <tr >        
                    <th style="padding-bottom: 50px;padding-left: 120px;font-size: 30px;color: blue;">Members of&nbsp; <font style="color: black;"><%=gname%></font></th>
    ...
    <%
                ...
                String userid="",fname="",lname="",imagen="";
                
                String lid=(String)session.getAttribute("Log_id");  
                int c=0;
                System.out.println("lid "+lid);
                String sq="select * from tbl_group where gname='"+gname+"'";     
                ResultSet rs=db.exquery(sq);           
                while(rs.next()){
                     userid=rs.getString("log_id");   
                String s1="select * from tbl_register where log_id='"+userid+"'";     
                ResultSet rs1=db.exquery(s1);           
                while(rs1.next()){
            ...
            <font style="padding-left: 50px;font-size: 20px;"><a><%=fname%>&nbsp;<%=lname%></a></font>
            </td>  
            </tr>    
