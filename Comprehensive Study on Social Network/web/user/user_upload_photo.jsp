...
<table>
     <tr><td><span class="blink_me"><h2>Images</h2></span></td></tr><tr>
<%
     Database qb=new Database();         
     String qe="select * from tbl_photo as r inner join tbl_register as g on r.user_id=g.log_id inner join tbl_login as l on l.login_id=r.user_id where r.user_id='"+id+"'";
     String w="select max(id) from tbl_photo where user_id='"+id+"'";
     ResultSet rx=qb.exquery(qe);
     ResultSet gv=qb.exquery(w);
     int index1=1;
     int k=0;
       while(gv.next())
     {
...
while(rx.next())
              {
                  %>
 <td><img src="../proimage/<%=rx.getString("image")%>" height="200" width="200"></a>
