...
String friend_id=request.getParameter("loginid");
...
temp=temp;
db.exupdate("insert into tbl_block(user_id,swear_word,swear_word_use,date,status)values('"+ids+"','"+bid+"','chat','"+today_date+"','0')");       
}
}
}
...
temp=temp.replace(arr[i], encoded);
temp=temp;
db.exupdate("insert into tbl_block(user_id,swear_word,swear_word_use,date,status)values('"+ids+"','"+qid+"','chat','"+today_date+"','0')");
}
}
}

db.exupdate("insert into tbl_chat(chat_message,chat_date,chat_user_login_id,chat_friend_login_id)values('"+temp+"','"+today_date+"','"+ids+"','"+friend_id+"')");
}
String query="select * from tbl_chat c inner join tbl_register r on c.chat_user_login_id=r.log_id where c.chat_user_login_id='"+ids+"' and c.chat_friend_login_id='"+friend_id+"' or c.chat_user_login_id='"+friend_id+"' and c.chat_friend_login_id='"+ids+"'";
ResultSet re=db.exquery(query);
while(re.next())
{
