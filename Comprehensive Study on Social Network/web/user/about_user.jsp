...
<div class="profile"  style="margin-top: 30px">                
   <%
String querys="select * from tbl_register where log_id='"+id+"'";
 String names="",date_of_birth="",gender="";
 ResultSet ress=db.exquery(querys);
 while(ress.next())
 {
     names=ress.getString("first_name")+" "+ress.getString("last_name");
     date_of_birth=ress.getString("date_of_birth");
     gender=ress.getString("gender");
 }
 ResultSet rx=db.exquery("select count(*) as c from tbl_request where user_id='"+id+"' and status='3'");
 String count="";
 while(rx.next())
 {
 count=rx.getString("c");
 }
%>
<div style="margin-left:40%;">
    <h3 style="color: #cc0000">Name of User:<%=names%></h3><br>
    <h3>Date Of Birth:<%=date_of_birth%></h3><br>
    <h3>Gender:<%=gender%></h3><br>
    <h3>No of Friends List:<%=count%></h3>
</div>
</div>                
